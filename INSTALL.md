...
-- Gemini files table for persistent PDF knowledge base
...
-- Enable RLS on gemini_files table and allow anonymous read access
ALTER TABLE gemini_files ENABLE ROW LEVEL SECURITY;

-- Allow all users (including anonymous) to read gemini_files
CREATE POLICY "Allow anonymous read access to gemini_files" 
ON gemini_files FOR SELECT 
TO anon, authenticated 
USING (true);

-- Only allow service role to insert/update/delete gemini_files
CREATE POLICY "Service role can manage gemini_files" 
ON gemini_files FOR ALL 
TO service_role 
USING (true);

-- RLS Reminder for other tables
-- Ensure RLS is enabled on other tables in your Supabase dashboard
